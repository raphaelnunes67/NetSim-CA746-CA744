...
def plot_energy_boxplots(circuit_name, bus_type, orientation='horizontal', figsize=(20, 24), file_format='png',
    ...
    if not any(data_by_mode[mode] for mode in control_modes):
        ...
        if orientation == 'horizontal':
            ...
            plt.ylabel('Energia (kWh)', fontsize=34)
            plt.tick_params(axis='y', labelsize=34)
            plt.tick_params(axis='x', labelsize=30, rotation=45)
        ...
        plt.grid(axis='y' if orientation == 'vertical' else 'x', linestyle='--', alpha=0.7, color='black')
        plt.tight_layout()
        plt.savefig(f'boxplot_energy_{circuit_name}_{bus_type}.{file_format}', format=file_format)
        print(f'Figure saved as boxplot_energy_{circuit_name}_{bus_type}.{file_format}')
