...
def run_ca746(pl_ev: int, pl_pv: int, db_path: str):
    log = Log()
...
def run_ca744(pl_ev: int, pl_pv: int, db_path: str):
    log = Log()
    simulation_logger = log.set_logger_file_and_stdout(f'SIMULATION_PL_EV_{pl_ev}_PL_PV_{pl_pv}')
...
def main(circuit, electric_vehicles, photovoltaics, database_path):
    normalized_path = normalize_path(database_path)
    if circuit == 'ca746':
        run_ca746(pl_ev=electric_vehicles, pl_pv=photovoltaics, db_path=normalized_path)
    elif circuit == 'ca744':
        run_ca744(pl_ev=electric_vehicles, pl_pv=photovoltaics, db_path=normalized_path)
